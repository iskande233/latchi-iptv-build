...
object YacineTvHelper {
    ...
    fun formatMatchTime(m: YacineMatch): String { val now=System.currentTimeMillis()/1000; val raw = when{now<m.startTime->{val sdf=java.text.SimpleDateFormat("HH:mm",java.util.Locale.US);sdf.format(java.util.Date(m.startTime*1000))}; now in m.startTime..m.endTime->{val e=((now-m.startTime)/60).toInt();if(e<=45)"${e}'" else if(e<=60)"HT" else "${e-15}'"}; else->"FT"}; return DigitNormalizer.normalizeDigits(raw) }
}
